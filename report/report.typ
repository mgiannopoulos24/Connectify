...
== Συνεχής Ολοκλήρωση & Παράδοση (CI/CD) με GitHub Actions <arch-ci>
...
=== Προηγμένες Λειτουργίες Ροής Αγγελιών και Αναρτήσεων
...
=== Εμπλουτισμένες Λειτουργίες Πραγματικού Χρόνου
...
=== Διαχείριση Ορατότητας Προφίλ και Σύστημα "Follow"
Για την παροχή μεγαλύτερου ελέγχου στους χρήστες, υλοποιήθηκαν δύο διακριτά συστήματα δικτύωσης.

- *Ορατότητα Προφίλ:* Οι χρήστες μπορούν να επιλέξουν αν το προφίλ τους θα είναι "public" ή "connections_only". Η λογική αυτή εφαρμόζεται στο `Accounts.get_user_profile!/2`, το οποίο φιλτράρει τα πεδία που επιστρέφονται στο frontend ανάλογα με τη ρύθμιση του χρήστη και το αν ο θεατής είναι ήδη συνδεδεμένος μαζί του.
...
=== Διαδικασία Onboarding Νέων Χρηστών
...
=== Ολοκληρωμένο Πάνελ Διαχείρισης (Admin Panel)
...
- *Εξαγωγή Δεδομένων:* Μια εξειδικευμένη σελίδα (`ExportPage.tsx`) που επιτρέπει στον διαχειριστή να επιλέξει συγκεκριμένους (ή όλους τους) χρήστες και να εξάγει τα δεδομένα τους σε μορφή JSON ή XML, καλύπτοντας πλήρως την απαίτηση 4 της εκφώνησης.
...
=== Προτεινόμενη Μέθοδος με Docker
Η εκτέλεση της εντολής `docker-compose up` από τον ριζικό φάκελο του project (όπως περιγράφεται στην ενότητα του backend) ξεκινά αυτόματα και τον Vite development server. Θα είναι διαθέσιμος στη διεύθυνση `https://localhost:8080`.

=== Χειροκίνητη Μέθοδος
1.  *Προαπαιτούμενα:* Εγκατάσταση του *Node.js* (έκδοση 20) και *npm*.
2.  *Ρύθμιση:*
    - Πλοήγηση στον φάκελο `frontend`.
    - Αντιγραφή του celf signed certificate και του certsigned key από τον φάκελο `backend/cert` στον φάκελο `frontend/cert` για την υποστήριξη HTTPS.
