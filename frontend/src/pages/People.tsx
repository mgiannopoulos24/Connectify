...
const PeoplePage: React.FC = () => {
    ...
    const discoverableUsers = useMemo(() => {
        ...
        const existingConnectionIds = new Set(
            currentUser.received_connections.filter(c => c.status === 'accepted').map(c => c.user_id)
            .concat(currentUser.sent_connections.filter(c => c.status === 'accepted').map(c => c.connected_user_id))
        );

        return users.filter(u =>
            u.id !== currentUser.id &&
            !existingConnectionIds.has(u.id)
        );
    }, [currentUser, users]);
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>Expand your professional network by connecting with others.</CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    {discoverableUsers.map(user => (
                        <Card key={user.id} className="flex flex-col items-center p-6 text-center">
                            <Link to={`/profile/${user.id}`}>
                                {user.photo_url ? (
                                    <img src={user.photo_url} alt={`${user.name} ${user.surname}`} className="h-24 w-24 rounded-full object-cover mb-4" />
                                ) : (
                            ...
                            <p className="text-sm text-gray-500 mb-4">{user.job_experiences?.[0]?.job_title || 'Professional'}</p>
